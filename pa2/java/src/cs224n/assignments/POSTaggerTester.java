...
public class POSTaggerTester {
   ...
   * arbitrary state, for example [DT, NN, 2], use the static factory
   * method State.buildState("DT", "NN", 2).  There isn't a single final
   * state, since sentences lengths vary, so State.getStopState(i) takes a
   * parameter for the length of the sentence.
   */
  static class State {
    ...
    private State(State state) {
      setState(state.getPreviousPreviousTag(),
    ...
    private static transient Interner<State> stateInterner = 
      new Interner<State>(new Interner.CanonicalFactory<State>() {
  ...
  static class POSTagger {
    ...
    // chop up the validation instances into local contexts and pass them
    // on to the local scorer.
    ...
    private List<LabeledLocalTrigramContext>
      extractLabeledLocalTrigramContexts(List<TaggedSentence> taggedSentences) {
    ...
    private List<LabeledLocalTrigramContext> 
      extractLabeledLocalTrigramContexts(TaggedSentence taggedSentence) {
      ...
      List<String> tags = 
        new BoundedList<String>(taggedSentence.getTags(), START_TAG, STOP_TAG);
      for (int position = 0; position <= taggedSentence.size() + 1; position++) {
        labeledLocalTrigramContexts.add
          (new LabeledLocalTrigramContext(words, position, tags.get(position - 2),
                                          tags.get(position - 1),
                                          tags.get(position)));
      }
      ...
      for (int position = 0; position <= sentence.size() + 1; position++) {
        ...
        for (State state : states) {
          ...
          LocalTrigramContext localTrigramContext = 
            new LocalTrigramContext(sentence,
                                    position,
                                    state.getPreviousPreviousTag(),
                                    state.getPreviousTag());
          Counter<String> tagScores =
            localTrigramScorer.getLogScoreCounter(localTrigramContext);
...
//        System.out.println("States: "+nextStates);
  ...
  static class LocalTrigramContext {
    ...
    public LocalTrigramContext(List<String> words,
                               int position,
                               String previousPreviousTag,
                               String previousTag) {
      this.words = words;
   ...
   * position -- basically a LabeledDatum
   */
  ...
  static class MostFrequentTagScorer implements LocalTrigramScorer {
    ...
    boolean restrictTrigrams; 
    // if true, assign log score of Double.NEGATIVE_INFINITY to illegal tag trigrams.
    ...
    public Counter<String> getLogScoreCounter(LocalTrigramContext localTrigramContext) {
      ...
      Set<String> allowedFollowingTags = 
        allowedFollowingTags(tagCounter.keySet(), 
    ...
    private Set<String> allowedFollowingTags(Set<String> tags, 
      ...
      for (String tag : tags) {
        String trigramString = 
          makeTrigramString(previousPreviousTag, previousTag, tag);
    ...
    public void train(List<LabeledLocalTrigramContext> labeledLocalTrigramContexts) {
      ...
      for (LabeledLocalTrigramContext labeledLocalTrigramContext : 
        ...
        seenTagTrigrams.add(makeTrigramString(labeledLocalTrigramContext.getPreviousPreviousTag(), 
  ...
  private static List<TaggedSentence> readTaggedSentences(String path, int low, int high) {
    ...
    for (Tree<String> tree : trees) {
      ...
      List<String> words = 
        new BoundedList<String>(new ArrayList<String>(tree.getYield()),
                                START_WORD,
                                STOP_WORD);
      List<String> tags =
        new BoundedList<String>(new ArrayList<String>(tree.getPreTerminalYield()), 
                                START_TAG,
                                STOP_TAG);
  ...
  private static void evaluateTagger(POSTagger posTagger,
    ...
    for (TaggedSentence taggedSentence : taggedSentences) {
      ...
      double scoreOfGuessedTagging =
        posTagger.scoreTagging(new TaggedSentence(words, guessedTags));
    ...
    System.out.println("Tag Accuracy: " + (numTagsCorrect / numTags) + 
                       " (Unknown Accuracy: " + (numUnknownWordsCorrect / numUnknownWords) + 
                       ")  Decoder Suboptimalities Detected: " + numDecodingInversions);
  }

  // pretty-print a pair of taggings for a sentence, possibly suppressing
  // the tags which correctly match
  ...
  public static void main(String[] args) {
    ...
    POSTagger posTagger = new POSTagger(new MostFrequentTagScorer(false),
                                        new GreedyDecoder<State>());
    posTagger.train(trainTaggedSentences);
    posTagger.validate(validationTaggedSentences);
    evaluateTagger(posTagger, testTaggedSentences, trainingVocabulary, true);
  }
